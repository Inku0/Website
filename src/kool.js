        ...
        return (
            <Container>
                <Grid container justify='center' align='center' spacing={3}>
                    <Selector data={{ size: 12, label: 'Mida soovite teisendada?', measurements: convert().measures(), populateType: 'mainSelector', populateWith: this.generateMainSelector(), selectedValue: this.state.mainSelector }} handleSelector={this.handleMainSelector} />
                    {
                        this.state.mainSelector ? (
                            <>
                                <Selector data={{ size: 6, label: 'Esimene ühik', populateType: 'firstQuantity', populateWith: convert().list(this.state.mainSelector), selectedValue: this.state.firstQuantity }} handleSelector={this.handleSelector} />
                                <Selector data={{ size: 6, label: 'Teine ühik', populateType: 'secondQuantity', populateWith: convert().list(this.state.mainSelector), selectedValue: this.state.secondQuantity }} handleSelector={this.handleSelector} />
                            </>
                        ) :(<></>)
                    }
                    {
